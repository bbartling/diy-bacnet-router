//! DIY BACnet Server — pure Rust axum entrypoint.
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| EnvFilter::new("info,diy_bacnet_server=info")),
        )
        .init();
...
async fn run(
    ...
    info!(
        "DIY BACnet Server (Rust) started (HTTP {}:{})",
...
async fn shutdown_signal(
    ...
    info!("DIY BACnet Server (Rust) stopped");
}
