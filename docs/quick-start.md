...
Run the management daemon on a workstation (this is **not** the Buildroot
appliance image):
...
| `GET /api/ws/metrics` | **WebSocket** — live trunk health snapshots (~1 Hz) |
...
{: .warning }
The dashboard is a **LAN-only operator console**. Do not expose it on the public
internet.

New to BACnet or MS/TP? Start with [Learn]({{ site.baseurl }}/learn/).
