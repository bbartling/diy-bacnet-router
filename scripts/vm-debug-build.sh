...
lock = pathlib.Path("config/upstream-lock.toml").read_text(encoding="utf-8")
expected = re.search(r'^revision = "([0-9a-f]{40})"', lock, re.M).group(1)
assert manifest["target"] == sys.argv[2]
assert manifest["project_rust_toolchain"] == "1.93.0"
assert manifest["rusty_bacnet"] == expected, (
    f"manifest rusty_bacnet={manifest['rusty_bacnet']!r} != lock {expected!r}"
)
